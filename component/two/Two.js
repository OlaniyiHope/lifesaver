import React from 'react';
import { StyleSheet, Text, View, TouchableOpacity, TextInput, ScrollView, SafeAreaView } from 'react-native';
import { Dropdown } from 'react-native-element-dropdown';
import Label from "react-native-label";
import styles from './style';
const Two = ({navigation}) => {
    let data = [
        {label: 'Why cant i', value: '1'},
        {label: 'How can i', value: '2'},
        {label: 'What is the', value: '3'},
        {label: 'How do i', value: '4'},
        {label: 'Why cant i', value: '5'},
   
    ];

    return(
        <SafeAreaView>
      <ScrollView>
<View style={styles.container}>

   <View style={styles.top}>
       <Text style={styles.questions}> Two factor authentication</Text>
   </View>
  
    <Text style={styles.question}>Set your security questions</Text>
   <TextInput style = {styles.input}
               placeholder = "Where do i live"/>
    <Text style={styles.question}>Enter your answer</Text>
   <TextInput style = {styles.input}
               placeholder = "Phone Number"/>
    <Text style={styles.question}>Enter your password to confirm</Text>
   <TextInput style = {styles.input}
               placeholder = "Phone Number"/>

   <View style={styles.bottom}>
   <TouchableOpacity style={styles.button} onPress={()=>navigation.navigate("Notify")}>
       <Text style={styles.buttonText}>Set questions</Text>
    </TouchableOpacity>
   
   {/*<TouchableOpacity onPress={()=>navigation.navigate("Topbar")}>
       <Text>End</Text>
    </TouchableOpacity>*/}
   </View>
 
  
  
 
    </View>
    </ScrollView>
    </SafeAreaView>
     

    )
}

export default Two;

